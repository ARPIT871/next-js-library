...
import { financeblog } from '../../../common/data/data';
    ...
    <Box>
        <Grid container spacing={2}>
          <Grid item xs={12}>
            <div>
              <h2>Finance Blog</h2>
              <p>This is the main content of your finance blog.</p>
            </div>
            <Box sx={{ flexGrow: 1, overflow: 'hidden', bgcolor: 'background.paper' }}>
              <Grid container spacing={4}>
                {financeblog.slice(0, visibleCount).map((image, index) => (
                  <Grid item key={index} xs={12} sm={6} md={6} lg={6}>
                    <Item>
                    <div style={{ position: 'relative' }}>
                    <CustomLink href={`/details/financeblog/${index}`} style={{ textDecoration: 'none' }}>
        ...
        {visibleCount < financeblog.length && (
