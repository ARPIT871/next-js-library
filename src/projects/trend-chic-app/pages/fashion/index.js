...
import { fashionblog } from '../../common/data/data';
      ...
      <Container maxWidth="lg"sx={{marginTop:'40px'}}>
        <Grid container spacing={2}>
          <Grid item xs={12} md={8}>
            <div>
              <h2>Fashion Blog</h2>
              <p>This is the main content of your fashion blog.</p>
            </div>
            <Box sx={{ flexGrow: 1, overflow: 'hidden', bgcolor: 'background.paper' }}>
              <Grid container spacing={4}>
                {fashionblog.slice(0, visibleCount).map((image, index) => (
                  <Grid item key={index} xs={12} sm={6} md={6} lg={6}>
                    <Item>
                    <div style={{ position: 'relative' }}>
                    <CustomLink href={`/details/fashionblog/${index}`} style={{ textDecoration: 'none' }}>
              ...
              {visibleCount < fashionblog.length && (
