...
import { uxdesignerblog } from '../../../common/data/data';
    ...
    <Box>
      <Container maxWidth="lg"sx={{marginTop:'40px'}}>
        <Grid container spacing={2}>
          <Grid item xs={12} >
            <div>
              <h2>UX Designer Blog</h2>
            </div>
            <Box sx={{ flexGrow: 1, overflow: 'hidden', bgcolor: 'background.paper' }}>
              <Grid container spacing={4}>
                {uxdesignerblog.slice(0, visibleCount).map((image, index) => (
                  <Grid item key={index} xs={12} sm={3} md={4} lg={4}>
                    <Item>
                    <ImageContainer>
                    <RouterLink to={`/details/uxdesignerblog/${index}`} style={{ textDecoration: 'none' }}>
              ...
              {visibleCount < uxdesignerblog.length && (
